...
#importing variables
tarr,thetaRK,omegaRK,thetaLS,omegaLS=np.loadtxt("energyDiffOut.csv", unpack=True, delimiter=',')
...
ax1.set_ylabel(r'$\Delta E/E_0\times 10^{-9}$ (4th order R-K)', color='#1f77b4')
ax1.set_ylim(np.array([0,14]))
ax1.tick_params('y', colors='#1f77b4')
ax1.plot(tarr*10**(-3)/(2*np.pi),(E_RK-E_RK[0])/E_RK[0]*10**(9), label="4th order R-K") #plot of total energy against time (relative to starting energy)
ax2 = ax1.twinx() #plot either result on separate axes of same graph (they differ by ~10^3)
ax2.plot(tarr*10**(-3)/(2*np.pi),(E_LS-E_LS[0])/E_LS[0]*10**(6), label='LSODA', color='#ff7f0e')
