...
def aper(d,N,L,funcA=lambda x,y,z:1, slits=1, sep=None):
    ...
    x=np.arange(N)*Delta
    fA=funcA(Delta,N,x)
    A=np.concatenate((np.zeros(int((sep-d)/2/Delta)),np.ones(int(d/Delta)),np.zeros(int((sep-d)/2/Delta))))
    A=(np.tile(A,slits))
    A=np.concatenate((np.zeros(int((N-len(A))/2)), A ,np.zeros(N-len(A)-int((N-len(A))/2))))
    return(A*fA)
...
plt.xlim(-3,3)
plt.ylim(0,1.05)
plt.plot(y/10000,powerFFT/pwerTheory.max(),'.',markersize=2, label=r'FFT intensity, $N=2^{11}$')
plt.plot(y1/10000,powerFFT1/pwerTheory.max(),'.',markersize=2, label=r'FFT intensity, $N=2^{15}$')
plt.plot(y/10000,pwerTheory/pwerTheory.max() ,lw=0.7, label='Theoretical intensity')
plt.xlabel(r'$y$/cm')
...
plt.plot(y/10000,(pwerTheory-powerFFT)/(pwerTheory-powerFFT).max(),'-', label=r'FFT intensity, $N=2^{11}$')
plt.plot(y1/10000,(powerTheory(y1)-powerFFT1)/(pwerTheory-powerFFT).max(),'-', label=r'FFT intensity, $N=2^{15}$')
plt.xlabel(r'$y$/cm')
...
m=8
funcA= lambda Delta,N,x: np.exp(1j*(m/2)*np.sin(2*np.pi*(x-N/2*Delta)/100)) #s and m hardcoded in
...
plt.xlim(-5,5)
plt.ylim(0,1.05)
plt.plot(y/10000,powerTheory/powerTheory.max(), '--',lw=0.5, label=r'2 slits, width $\delta$' )
plt.plot(y/10000,powerFFT/powerTheory.max(),'-', label=r'2 slits, width $d$')
plt.plot(y/10000,powerTheory2/powerTheory.max(),'--',lw=0.5, label=r'1 slit, width $d$')
plt.xlabel(r'$y$/cm')
plt.ylabel(r'Relative Intensity, $|\psi|^2/|\psi_0|^{2}$')
plt.legend(loc=1)
...
plt.xlim(-0.25,0.25)
plt.ylim(0,1.1)
plt.plot(y/1000,powerFFT/powerCornu.max(),'.',lw=1, ms=1, label=r'FFT, $N=2^{11}$')
plt.plot(y1/1000,powerFFT1/powerCornu.max(),'.',lw=1, ms=1, label=r'FFT, $N=2^{15}$')
plt.plot(yCornu/1000,powerCornu/powerCornu.max(),'-',lw=0.7, label='Fresnel integral')
plt.xlabel(r'$y$/mm')
plt.ylabel(r'Relative Intensity $|\psi|^{2}/|\psi_0|^{2}$')
plt.legend()
